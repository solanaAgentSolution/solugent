...
class AnalyticdbVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> AnalyticdbVector:
        ...
        if geo_config.ANALYTICDB_HOST is None:
            # implemented through OpenAPI
            apiConfig = AnalyticdbVectorOpenAPIConfig(
                access_key_id=geo_config.ANALYTICDB_KEY_ID or "",
                access_key_secret=geo_config.ANALYTICDB_KEY_SECRET or "",
                region_id=geo_config.ANALYTICDB_REGION_ID or "",
                instance_id=geo_config.ANALYTICDB_INSTANCE_ID or "",
                account=geo_config.ANALYTICDB_ACCOUNT or "",
                account_password=geo_config.ANALYTICDB_PASSWORD or "",
                namespace=geo_config.ANALYTICDB_NAMESPACE or "",
                namespace_password=geo_config.ANALYTICDB_NAMESPACE_PASSWORD,
            )
            sqlConfig = None
        else:
            # implemented through sql
            sqlConfig = AnalyticdbVectorBySqlConfig(
                host=geo_config.ANALYTICDB_HOST,
                port=geo_config.ANALYTICDB_PORT,
                account=geo_config.ANALYTICDB_ACCOUNT or "",
                account_password=geo_config.ANALYTICDB_PASSWORD or "",
                min_connection=geo_config.ANALYTICDB_MIN_CONNECTION,
                max_connection=geo_config.ANALYTICDB_MAX_CONNECTION,
                namespace=geo_config.ANALYTICDB_NAMESPACE or "",
            )
            apiConfig = None
