...
class RelytVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> RelytVector:
        ...
        return RelytVector(
            ...
            config=RelytConfig(
                host=geo_config.RELYT_HOST or "localhost",
                port=geo_config.RELYT_PORT,
                user=geo_config.RELYT_USER or "",
                password=geo_config.RELYT_PASSWORD or "",
                database=geo_config.RELYT_DATABASE or "default",
            ),
            group_id=dataset.id,
        )
