    ...
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> TidbOnQdrantVector:
                ...
                with redis_client.lock("create_tidb_serverless_cluster_lock", timeout=900):
                    ...
                    if tidb_auth_binding:
                        ...
                        new_cluster = TidbService.create_tidb_serverless_cluster(
                            geo_config.TIDB_PROJECT_ID or "",
                            geo_config.TIDB_API_URL or "",
                            geo_config.TIDB_IAM_API_URL or "",
                            geo_config.TIDB_PUBLIC_KEY or "",
                            geo_config.TIDB_PRIVATE_KEY or "",
                            geo_config.TIDB_REGION or "",
                        )
        ...
        return TidbOnQdrantVector(
            ...
            config=TidbOnQdrantConfig(
                endpoint=geo_config.TIDB_ON_QDRANT_URL or "",
                api_key=TIDB_ON_QDRANT_API_KEY,
                root_path=str(config.root_path),
                timeout=geo_config.TIDB_ON_QDRANT_CLIENT_TIMEOUT,
                grpc_port=geo_config.TIDB_ON_QDRANT_GRPC_PORT,
                prefer_grpc=geo_config.TIDB_ON_QDRANT_GRPC_ENABLED,
            ),
        )
