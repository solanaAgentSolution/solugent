...
SSRF_DEFAULT_MAX_RETRIES = geo_config.SSRF_DEFAULT_MAX_RETRIES

proxy_mounts = (
    {
        "http://": httpx.HTTPTransport(proxy=geo_config.SSRF_PROXY_HTTP_URL),
        "https://": httpx.HTTPTransport(proxy=geo_config.SSRF_PROXY_HTTPS_URL),
    }
    if geo_config.SSRF_PROXY_HTTP_URL and geo_config.SSRF_PROXY_HTTPS_URL
    else None
)
...
def make_request(method, url, max_retries=SSRF_DEFAULT_MAX_RETRIES, **kwargs):
    ...
    if "timeout" not in kwargs:
        kwargs["timeout"] = httpx.Timeout(
            timeout=geo_config.SSRF_DEFAULT_TIME_OUT,
            connect=geo_config.SSRF_DEFAULT_CONNECT_TIME_OUT,
            read=geo_config.SSRF_DEFAULT_READ_TIME_OUT,
            write=geo_config.SSRF_DEFAULT_WRITE_TIME_OUT,
        )
    ...
    while retries <= max_retries:
        try:
            if geo_config.SSRF_PROXY_ALL_URL:
                with httpx.Client(proxy=geo_config.SSRF_PROXY_ALL_URL) as client:
