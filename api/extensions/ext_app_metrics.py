...
def init_app(app: geoApp):
    ...
    def after_request(response):
        ...
        response.headers.add("X-Version", geo_config.CURRENT_VERSION)
        response.headers.add("X-Env", geo_config.DEPLOY_ENV)
        return response

    @app.route("/health")
    def health():
        return Response(
            json.dumps({"pid": os.getpid(), "status": "ok", "version": geo_config.CURRENT_VERSION}),
            status=200,
