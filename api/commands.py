...
def reset_encrypt_key_pair():
    ...
    Only support SELF_HOSTED mode.
    """
    if geo_config.EDITION != "SELF_HOSTED":
...
def migrate_knowledge_vector_database():
    ...
    skipped_count = 0
    total_count = 0
    vector_type = geo_config.VECTOR_STORE
...
def add_qdrant_doc_id_index(field: str):
    ...
    try:
        ...
        for binding in bindings:
            if geo_config.QDRANT_URL is None:
                raise ValueError("Qdrant URL is required.")
            qdrant_config = QdrantConfig(
                endpoint=geo_config.QDRANT_URL,
                api_key=geo_config.QDRANT_API_KEY,
                root_path=current_app.root_path,
                timeout=geo_config.QDRANT_CLIENT_TIMEOUT,
                grpc_port=geo_config.QDRANT_GRPC_PORT,
                prefer_grpc=geo_config.QDRANT_GRPC_ENABLED,
            )
            try:
