...
import MogeoRetrievalModal from './mogeo-retrieval-modal'
...
const HitTesting: FC<Props> = ({ datasetId }: Props) => {
  ...
  const [isShowMogeoRetrievalModal, setIsShowMogeoRetrievalModal] = useState(false)
    ...
    <div className={s.container}>
      <div className='px-6 py-3 flex flex-col'>
        ...
        <Textarea
          ...
          text={text}
          isExternal={isExternal}
          onClickRetrievalMethod={() => setIsShowMogeoRetrievalModal(true)}
      ...
      <Drawer unmount={true} isOpen={isShowMogeoRetrievalModal} onClose={() => setIsShowMogeoRetrievalModal(false)} footer={null} mask={isMobile} panelClassname='mt-16 mx-2 sm:mr-2 mb-3 !p-0 !max-w-[640px] rounded-xl'>
        <MogeoRetrievalModal
          indexMethod={currentDataset?.indexing_technique || ''}
          value={retrievalConfig}
          isShow={isShowMogeoRetrievalModal}
          onHide={() => setIsShowMogeoRetrievalModal(false)}
          onSave={(value) => {
            setRetrievalConfig(value)
            setIsShowMogeoRetrievalModal(false)
          }}
        />
      </Drawer>
    </div>
